...
MUST_JOIN_CHANNEL_1 = "@jp_network"
MUST_JOIN_CHANNEL_2 = "@AboutXjpExo"

# LOG CHANNEL
LOG_CHANNEL_ID = "-1003979481171"

# Referral commission percentage
REFERRAL_COMMISSION = 1.5

# Global API Credentials for Pyrogram Login
GLOBAL_API_ID = 35094834
GLOBAL_API_HASH = "961be8ee9d4bb1462d0ec6539fd40ea6"
...
def is_admin(user_id):
    try:
        return int(user_id) in [ADMIN_ID, PARTNER_ID]
    except:
        return False
            ...
            if is_admin(user_id):
                ...
                if action == "approve_rech":
                    ...
                    recharges_col.update_one(
                        {"req_id": req_id},
                        {"$set": {"status": "approved", "processed_at": datetime.utcnow(), "processed_by": user_id}}
                    )
...
def process_recharge_amount(msg):
    try:
        ...
        if amount < 1:
            bot.send_message(msg.chat.id, "❌ Minimum recharge is ₹20. Enter amount again:")
    ...
    try:
        ...
        try:
            bot.send_message(
                ...
                "Contact admin @NAKS4PANDIT if you believe this is a mistake."
            )
        except:
            pass
    except ValueError:
...
def show_countries(chat_id):
    ...
    # Automatic country, price and stock display
    text = """🌍 <b>Choose Your Country</b>

⚡ <b>Current Exchange:</b> 1 USDT ≈ ₹89

📱 <b>Available Accounts & Prices</b>
...
"""
    ...
    for country in countries:
        ...
        # Automatic INR to USDT conversion
        usdt_price = float(price) / 89
        
        text += (
            f"🌍 {country_name} — "
            f"${usdt_price:.2f} ({format_currency(price)}) "
            f"📦 Stock: {accounts_count}\n"
        )
    
    text += """
👇 <b>Select your preferred country below:</b>"""
    
    # Automatic country buttons
    markup = InlineKeyboardMarkup(row_width=2)
